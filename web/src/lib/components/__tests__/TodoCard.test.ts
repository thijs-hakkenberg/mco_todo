...
describe('TodoCard Component', () => {
  ...
  it('should apply different colors for different priorities', () => {
    ...
    priorities.forEach(({ priority, class: className }) => {
      ...
      const { container } = render(TodoCard, { props: { todo } });
  ...
  it('should not be draggable when disabled', () => {
    const { container } = render(TodoCard, { props: { todo: mockTodo, draggable: false } });
  ...
  it('should show completed state for done todos', () => {
    ...
    const { container } = render(TodoCard, { props: { todo: doneTodo } });
  ...
  it('should show blocked indicator for blocked todos', () => {
    ...
    const { getByText } = render(TodoCard, { props: { todo: blockedTodo } });
  ...
  it('should handle todos without optional fields', () => {
    ...
    const { getByText, queryByText } = render(TodoCard, { props: { todo: minimalTodo } });
  ...
  it('should show progress bar for in-progress todos', () => {
    ...
    const { container } = render(TodoCard, { props: { todo: inProgressTodo, progress: 65 } });
